...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Group per debt dan hitung remainingCalculated
    ...
    for (const [, group] of groupedByDebt.entries()) {
      ...
      for (const p of group.items) {
        ...
        result.push({
          ...
          totalRemaining: 0, // placeholder sementara
        });
      }
    }

    // 🔢 Hitung total remaining global per user
    ...
    // Gabungkan ke setiap item di result
...
export async function POST(req: Request) {
  try {
    ...
    console.log("BODY DITERIMA:", body); // Tambahkan ini
    const parsed = paymentSchema.safeParse(body);

    if (!parsed.success) {
      const errorMessage = parsed.error.errors[0];
      console.log("PARSED:", parsed); // Tambahkan ini
      return NextResponse.json(
        {
          success: false,
    ...
    // Ambil semua utang user yg belum lunas (sisa > 0), urut dari yg paling lama
    ...
    // Jika masih ada sisa uang yang tidak bisa dialokasikan ke utang apa pun
